...
interface AgentPlanProps {
  /** Controlled task list. When provided, statuses are driven by the parent
   *  (e.g. live reconcile stages); demo data + click-to-toggle are used
   *  otherwise. */
  tasks?: Task[];
  /** Task ids expanded by default. */
  defaultExpanded?: string[];
}

export default function AgentPlan({ tasks: externalTasks, defaultExpanded }: AgentPlanProps) {
  // Fully controlled when `tasks` is provided (parent drives statuses, e.g.
  // live reconcile stages); otherwise internal demo state with toggles.
  const [internalTasks, setInternalTasks] = useState<Task[]>(initialTasks);
  const tasks = externalTasks ?? internalTasks;
  const [expandedTasks, setExpandedTasks] = useState<string[]>(defaultExpanded ?? ["1"]);
  ...
  // Toggle task status (demo mode only — controlled mode is parent-driven)
  ...
  // Toggle subtask status (demo mode only — controlled mode is parent-driven)
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          >
                            {task.status === "completed" ? (
                              ...
                              <CircleDotDashed className="h-4.5 w-4.5 text-blue-500 animate-[spin_1.8s_linear_infinite]" />
                                      ...
                                      <AnimatePresence mode="wait">
                                        ...
                                        >
                                          {subtask.status === "completed" ? (
                                            ...
                                            <CircleDotDashed className="h-3.5 w-3.5 text-blue-500 animate-[spin_1.8s_linear_infinite]" />
